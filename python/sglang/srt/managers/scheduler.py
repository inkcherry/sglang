...
class Scheduler(
    ...
    @DynamicGradMode()
    def _eager_refill(self, prev_running: int):
        """When the running batch shrinks significantly, eagerly receive new requests
        to minimize GPU idle time between wave completions."""
        cur_running = len(self.running_batch.reqs) if not self.running_batch.is_empty() else 0
        if prev_running > 4 and cur_running < prev_running * 0.5:
    ...
    def event_loop_normal(self):
        ...
        while True:
            ...
            if batch:
                prev_running = len(self.running_batch.reqs) if not self.running_batch.is_empty() else 0
                result = self.run_batch(batch)
                self.process_batch_result(batch, result)
                self._eager_refill(prev_running)
            else:
    ...
    def recv_requests(
        ...
        self.send_to_detokenizer.flush()
